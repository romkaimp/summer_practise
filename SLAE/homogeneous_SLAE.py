...
py_handler = handlers.RotatingFileHandler(f"{__name__}.log", mode='w', maxBytes=1024*2)
...
def param_check(r, n, k) -> bool:
    ...
    if r - n < 1:
        infoLogger.error(f"r({r}) - n({n}) < 1")
        raise FalseParameters(f"r({r}) - n({n}) < 1")
    elif k < r - n:
...
def generate_random_matrix(rows, cols, k, seed=seed_value, start_variant=start_variant) -> np.ndarray:
    ...
    for i in range(rows, k):
        ...
        infoLogger.info(f"new_row: row={matrix2} coefficients={coeffs}")
